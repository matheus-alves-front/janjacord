...
class DesktopInstance {
  ...
  sendText(channelId, text) {
    ...
    return new Promise((resolve) => {
      ...
      this.client.send("envelope.send", env);
    });
  }
...
async function main() {
  ...
  try {
    ...
    // listener do bob registrado ANTES do push (evento é síncrono no host)
    const welcomePromise = bob.waitEvent();
    ...
    assert(welcomePush.ok, "welcome enviado ao bob");

    console.log("[smoke-core] bob entra no grupo MLS…");
    const welcomeEvt = await welcomePromise;
    assert(welcomeEvt?.type === "welcome.deliver", "bob recebe welcome");
    const joined = JSON.parse(mls.join_group(bob.identity.seed.toString("hex"), bob.identity.identityId, welcomeEvt.welcomeB64));
    ...
    console.log("[smoke-core] E2EE reverso: alice → bob…");
    const aliceDeliverPromise = bob.waitEvent();
    const sent2 = await alice.sendText(general.id, "oi bob, criptografia real");
    assert(sent2.ok, "alice envia envelope (reverso)");
    const deliver2 = await aliceDeliverPromise;
    const plain2 = bob.decrypt(deliver2.envelope);
    assert(plain2 === "oi bob, criptografia real", `bob decifra E2EE: "${plain2}"`);

    // consome a segunda mensagem também (purge completo)
    await alice.command({ type: "message.ackConsumed", messageId: deliver2.envelope.messageId });
    await bob.command({ type: "message.ackConsumed", messageId: deliver2.envelope.messageId });
    ...
    const fakeImg = Buffer.from("PNG-DADOS-FICTICIOS-1234567890");
    ...
    const up = await alice.command({ type: "attachment.upload", assetId, data: encA.toString("base64"), sizeBytes: fakeImg.length });
    assert(up.ok, "upload de chunk cifrado ao host");
    // mensagem MLS com payload {text, attachments, assetKeys}
    const payload = JSON.stringify({
      text: "📎 foto.png",
      attachments: [{ assetId, name: "foto.png", mimeType: "image/png", sizeBytes: fakeImg.length, totalChunks: 1, hash: "h" }],
    ...
    const envA = buildEnvelope({
      ...
      audience: { algo: "sha256", commitment: "", members: bob.serverState.members.map((m) => m.identityId) },
    ...
    await new Promise((resolve) => {
      ...
      bob.client.onEventOnce("result", (f) => { clearTimeout(t); resolve(f.data); });
      bob.client.send("envelope.send", envA);
    });
    ...
    assert(dl.ok, "download do chunk cifrado");
    const encB = Buffer.from(dl.data.data, "base64");
